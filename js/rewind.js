...
function mainChange(obj1) {
  ...
  obj.btnAppend = function () {
    $(obj.btnDiv).append(
      '<img src="/Public/images/rewind/btnOn.png" onclick="' +
      obj.btnEvent +
      '(0)" />'
    );
    for (var i = 1; i <= obj.mun; i++) {
      $(obj.childDiv + ":eq(" + i + ")").css(
        "right",
        i * obj.childWidth + "px"
      );
      $(obj.btnDiv).append(
        '<img src="/Public/images/rewind/btn.png" onclick="' +
        obj.btnEvent +
        "(" +
        i +
        ')" />'
    ...
    $(obj.childDiv)
      .not($(obj.childDiv + ":eq(" + obj.nowIndex + ")"))
      .css("right", obj.childWidth + "px");
    $(obj.childDiv + ":eq(" + obj.mun + ")").css(
      "right",
      -obj.childWidth + "px"
    );
  ...
  obj.btnPosition = function (btnIndex) {
    ...
    $(obj.btnDiv + ">img:eq(" + btnIndex + ")").attr(
      "src",
      "/Public/images/rewind/btnOn.png"
    );
  };
  //切换事件
...
function mainChild5BtnClick(btnIndex) {
  ...
  if (btnIndex == "-") {
    mainChild5.nowIndex == 0 ?
      (btnIndex = mainChild5.mun) :
      (btnIndex = mainChild5.nowIndex - 1);
    mainChild5.bannerDo(btnIndex, "-");
    return;
  } else if (btnIndex == "+") {
    mainChild5.nowIndex == mainChild5.mun ?
      (btnIndex = 0) :
      (btnIndex = mainChild5.nowIndex + 1);
    mainChild5.bannerDo(btnIndex, "+");
    return;
...
$(function () {
  ...
  $("#mainChild5 .contents")
    .mouseover(function () {
      $(this).find(".leftBtn").animate({
        left: "14px"
      }, 300);
      $(this).find(".rightBtn").animate({
        right: "14px"
      }, 300);
    })
    .mouseleave(function () {
